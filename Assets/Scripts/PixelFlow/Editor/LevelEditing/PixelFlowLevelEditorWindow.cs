...
{
    public sealed class PixelFlowLevelEditorWindow : EditorWindow
    {
        private const string DefaultDatabaseFolder = "Assets/Data/PixelFlow";
        private const string DefaultDatabaseAssetName = "PixelFlowLevelDatabase";
        private const string DefaultSettingsFolder = "Assets/Settings";
        private const string DefaultBlockDataAssetPath = DefaultSettingsFolder + "/PixelFlowDefaultBlockData.asset";
        ...
        [SerializeField] private PixelFlowLevelDatabase selectedDatabase;
        ...
        [SerializeField] private PixelFlowBlockData selectedBlockData;
        ...
        [SerializeField] private List<PixelFlowPlacedObjectData> workingPlacedObjects = new();
        ...
        [SerializeField] private int selectedPigQueuePrimaryIndex = -1;
        [SerializeField] private int selectedPigQueueSecondaryIndex = -1;
        [SerializeField] private int selectedPigQueueSwapAmount = 1;
        ...
        private int pigQueueDragSourceIndex = -1;
        private int pigQueueDragHoverLinearIndex = -1;
        private Vector2 pigQueueDragStartMousePosition;
        private bool isPigQueueDragging;

        [MenuItem("Tools/Pixel Flow/Level Editor")]
        private static void Open()
        {
            var window = GetWindow<PixelFlowLevelEditorWindow>("Pixel Flow Level Editor");
        ...
        private void EnsureEditorDefaults()
        {
            ...
            workingPlacedObjects ??= new List<PixelFlowPlacedObjectData>();
        ...
        private void DrawHeader()
        {
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                var nextDatabase = (PixelFlowLevelDatabase)EditorGUILayout.ObjectField(
                    "Level Database",
                    selectedDatabase,
                    typeof(PixelFlowLevelDatabase),
                    false);
        ...
        private void DrawImportSettings()
        {
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                EditorGUI.BeginChangeCheck();
                var nextBlockData = (PixelFlowBlockData)EditorGUILayout.ObjectField("Block Data", selectedBlockData, typeof(PixelFlowBlockData), false);
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    selectedBlockData = nextBlockData;
        ...
        private void GenerateTemporaryTestLevel()
        {
            ...
            selectedDatabase.EnsureConsistency();
            if (!TryResolveTestGenerationContext(out var environment, out var tempParent, out var sceneContext, out var contextMessage))
            {
            ...
            var tempRoot = CreateTemporaryTestLevelRoot(tempParent);
            var generatedBlockCount = 0;
            var generatedPigCount = 0;
            var skippedPigColors = new HashSet<PigColor>();

            var boardRoot = CreateTemporaryChildRoot(tempRoot.transform, TemporaryTestBoardRootName, environment != null ? environment.BlockContainer : null);
            var deckRoot = CreateTemporaryChildRoot(tempRoot.transform, TemporaryTestDeckRootName, environment != null ? environment.DeckContainer : null);
            ...
            if (boardRoot != null)
            {
                generatedBlockCount = GenerateTemporaryBoard(boardRoot, sceneContext);
            }
        ...
        private bool TryResolveTestGenerationContext(
            out PixelFlowEnvironmentContext environment,
            out Transform tempParent,
            out PixelFlowSceneContext sceneContext,
            out string message)
        {
            environment = null;
            tempParent = null;
            ...
            if (!TryResolveEnvironmentFromSceneContext(out environment, out message))
            {
                return false;
            }
            ...
            if (environment == null)
            {
                message = "Could not resolve a PixelFlowEnvironmentContext in the open scene.";
                return false;
            }

            sceneContext = environment.GetComponentInParent<PixelFlowSceneContext>();
            if (sceneContext == null)
            {
                var sceneContexts = UnityEngine.Object.FindObjectsByType<PixelFlowSceneContext>(FindObjectsInactive.Include, FindObjectsSortMode.None);
                for (int i = 0; i < sceneContexts.Length; i++)
                {
                    var candidate = sceneContexts[i];
                    if (candidate == null || candidate.gameObject.scene != environment.gameObject.scene)
                    {
                        continue;
                    }
            ...
            tempParent = sceneContext != null ? sceneContext.transform : environment.transform;
            if (tempParent == null)
            {
        ...
        private GameObject CreateTemporaryTestLevelRoot(Transform parent)
        {
            var root = new GameObject(TemporaryTestLevelRootName);
            root.transform.SetParent(parent, false);
            ApplyTemporaryHideFlagsRecursive(root);
            return root;
        }

        private Transform CreateTemporaryChildRoot(Transform parent, string childName, Transform reference)
        {
            if (parent == null)
            {
                return null;
        ...
        private int GenerateTemporaryBoard(Transform boardRoot, PixelFlowSceneContext sceneContext)
        {
            ...
            var blockPrefab = ResolveTemporaryBlockPrefab(sceneContext);
            if (blockPrefab == null)
            {
                return 0;
            }
            ...
            var width = Mathf.Max(1, workingGridSize.x);
            var height = Mathf.Max(1, workingGridSize.y);
            var environment = sceneContext != null ? sceneContext.EnsureEnvironment() : null;
            var layout = PixelFlowBoardFitUtility.ResolvePlacedObjectLayout(
                environment,
                boardRoot,
            ...
            for (int placementIndex = 0; placementIndex < workingPlacedObjects.Count; placementIndex++)
            {
                ...
                var pigColor = definition.Kind == PixelFlowPlaceableKind.Pig ? definition.Color : PigColor.None;
                var fallbackColor = definition.Kind == PixelFlowPlaceableKind.Pig
                ...
                for (int offsetX = 0; offsetX < size.x; offsetX++)
                {
                    for (int offsetY = 0; offsetY < size.y; offsetY++)
                    {
                        ...
                        ApplyTemporaryHideFlagsRecursive(instance);
        ...
        private int GenerateTemporaryDeck(Transform deckRoot, PixelFlowEnvironmentContext environment, ISet<PigColor> skippedPigColors)
        {
        ...
        private static float ResolveTemporaryDeckDepthDirection(Transform deckRoot, PixelFlowEnvironmentContext environment)
        {
        ...
        private GameObject ResolveTemporaryBlockPrefab(PixelFlowSceneContext sceneContext)
        {
            ...
            if (sceneContext?.Theme != null && sceneContext.Theme.BlockPrefab != null)
            {
        ...
        private GameObject ResolveTemporaryPigPrefab(PigColor color, bool preferGeneric)
        {
            ...
            if (selectedDatabase != null)
            {
                for (int i = 0; i < selectedDatabase.PlaceableObjects.Count; i++)
                {
                    ...
                    if (candidate != null
                        && candidate.Kind == PixelFlowPlaceableKind.Pig
        ...
        private static bool IsValidTemporaryPigPrefab(GameObject prefab)
        {
            ...
            return (prefab.GetComponentInChildren<PixelFlowPigController>(true) != null
                    && prefab.GetComponentInChildren<PixelFlowPigView>(true) != null)
                || (prefab.GetComponentInChildren<PixelFlowPoolableVisual>(true) != null
                    && prefab.GetComponentInChildren<PixelFlowVisualView>(true) != null)
                || (prefab.GetComponentInChildren<PixelFlowAtlasColorTarget>(true) != null
        ...
        private static bool TryConfigureTemporaryPig(GameObject instance, PigQueueEntry entry, Color fallbackColor)
        {
            ...
            var pigController = instance.GetComponent<PixelFlowPigController>();
            ...
            var pigVisual = instance.GetComponent<PixelFlowPoolableVisual>();
            if (pigVisual != null)
            {
                pigVisual.ConfigurePig(entry.Color, entry.Ammo);
                pigVisual.HideTray();
                ApplyTemporaryColor(instance, entry.Color, fallbackColor);
                return true;
            }

            var atlasTarget = instance.GetComponentInChildren<PixelFlowAtlasColorTarget>(true);
        ...
        private static void ApplyTemporaryHideFlagsRecursive(GameObject gameObject)
        {
            if (gameObject == null)
            {
                return;
        ...
        private void DrawCell(Rect rect, Vector2Int gridPosition, int placementIndex, bool isRoot)
        {
            ...
            {
                fillColor = definition.Kind == PixelFlowPlaceableKind.Obstacle
                ...
                for (int placementIndex = 0; placementIndex < workingPlacedObjects.Count; placementIndex++)
                {
                    ...
                    for (int dx = 0; dx < size.x; dx++)
                    {
                        for (int dy = 0; dy < size.y; dy++)
                        {
                            ...
                            if (definition.Kind == PixelFlowPlaceableKind.Pig && definition.Color != PigColor.None)
                            {
        ...
        private static Color32 GetPreviewColor(PixelFlowPlaceableDefinition definition)
        {
            var color = definition.Kind == PixelFlowPlaceableKind.Obstacle
        ...
        private void EnsurePlacedObjectCapacity(int minimumCapacity)
        {
            ...
            {
                workingPlacedObjects = new List<PixelFlowPlacedObjectData>(minimumCapacity);
                return;
            }
        ...
        private void ApplyDatabaseSelection(PixelFlowLevelDatabase database)
        {
        ...
        private void CreateLevelDatabase()
        {
            ...
            var database = CreateInstance<PixelFlowLevelDatabase>();
        ...
        private PixelFlowLevelData CreateWorkingLevelSnapshot()
        {
            NormalizeWorkingPigQueueSlots();

            var snapshot = new PixelFlowLevelData
            {
        ...
        private void ImportImageToGrid()
        {
            ...
            if (!PixelFlowImageImporter.TryImport(sourceImage, workingImportSettings, out var importedGrid, out var error))
            {
                if (!string.IsNullOrEmpty(sourceImageAssetPath))
                {
                    sourceImage = LoadTextureAsset(sourceImageAssetPath) ?? sourceImage;
            ...
            if (workingPlacedObjects == null)
            {
                workingPlacedObjects = new List<PixelFlowPlacedObjectData>(estimatedPlacementCapacity);
            }
            else
            {
                workingPlacedObjects.Clear();
            ...
            for (int x = 0; x < importedGrid.GetLength(0); x++)
            {
                for (int yView = 0; yView < importedGrid.GetLength(1); yView++)
                {
                    ...
                    workingPlacedObjects.Add(new PixelFlowPlacedObjectData(placeable.Id, new Vector2Int(x, yGrid)));
                }
            }
        ...
        private void FillGridWithBrush(PigColor brushColor)
        {
            ...
            {
                statusMessage = $"{brushColor} için database içinde 1x1 pig tanımı bulunamadı.";
                return;
            }
            ...
            for (int y = 0; y < workingGridSize.y; y++)
            {
                ...
                {
                    workingPlacedObjects.Add(new PixelFlowPlacedObjectData(brush.Id, new Vector2Int(x, y)));
                }
            }
        ...
        private void TryPlaceBrush(Vector2Int gridPosition, bool isDrag)
        {
            ...
            {
                statusMessage = $"{selectedBrushColor} için database içinde 1x1 pig tanımı bulunamadı.";
                return;
            }
            ...
            if (TryFindPlacementIndexAt(gridPosition, out var existingPlacementIndex))
            {
                    ...
                    && existingPlacedObject.Origin == gridPosition)
                {
                    ...
                    workingPlacedObjects[existingPlacementIndex] = new PixelFlowPlacedObjectData(brush.Id, gridPosition);
                    InvalidateGridPreviewCache();
            ...
            workingPlacedObjects.Add(new PixelFlowPlacedObjectData(brush.Id, gridPosition));
            InvalidateGridPreviewCache();
        ...
        private bool TryFindPlacementAtOrigin(Vector2Int origin, PixelFlowPlaceableId placeableId, out int placementIndex)
        {
            for (int i = 0; i < workingPlacedObjects.Count; i++)
            {
        ...
        private bool TryResolvePlacedObject(int placementIndex, out PixelFlowPlacedObjectData placedObject, out PixelFlowPlaceableDefinition definition)
        {
        ...
        {
            selectedPigQueuePrimaryIndex = -1;
            selectedPigQueueSecondaryIndex = -1;
            selectedPigQueueSwapAmount = Mathf.Max(1, GetAmmoStep());
        }
        ...
        private void EnsurePigQueueSelectionValid()
        {
            ...
            if (!IsValidPigQueueIndex(selectedPigQueuePrimaryIndex))
            {
                selectedPigQueuePrimaryIndex = -1;
            }

            if (!IsValidPigQueueIndex(selectedPigQueueSecondaryIndex)
                || selectedPigQueueSecondaryIndex == selectedPigQueuePrimaryIndex)
            {
                selectedPigQueueSecondaryIndex = -1;
            }

            if (selectedPigQueuePrimaryIndex >= 0
                && selectedPigQueueSecondaryIndex >= 0
                && workingPigQueue[selectedPigQueuePrimaryIndex].Color != workingPigQueue[selectedPigQueueSecondaryIndex].Color)
            {
                selectedPigQueueSecondaryIndex = -1;
            }

            selectedPigQueueSwapAmount = Mathf.Max(GetAmmoStep(), SnapDownToStep(Mathf.Max(GetAmmoStep(), selectedPigQueueSwapAmount), GetAmmoStep()));
        }
        ...
        {
            return queueIndex == selectedPigQueuePrimaryIndex
                || queueIndex == selectedPigQueueSecondaryIndex;
        }

        private string GetPigQueuePreviewSelectionLabel(int queueIndex)
        {
            if (queueIndex == selectedPigQueuePrimaryIndex)
            {
                return "A";
            }

            if (queueIndex == selectedPigQueueSecondaryIndex)
            {
                return "B";
            }
        ...
        private void SelectPigQueuePreviewEntry(int queueIndex)
        {
            ...
            if (!IsValidPigQueueIndex(queueIndex))
            {
                return;
            }

            if (selectedPigQueuePrimaryIndex == queueIndex)
            {
                selectedPigQueuePrimaryIndex = selectedPigQueueSecondaryIndex;
                selectedPigQueueSecondaryIndex = -1;
                return;
            }

            if (selectedPigQueueSecondaryIndex == queueIndex)
            {
                selectedPigQueueSecondaryIndex = -1;
                return;
            }
            ...
            if (selectedPigQueuePrimaryIndex < 0)
            {
                selectedPigQueuePrimaryIndex = queueIndex;
                selectedPigQueueSecondaryIndex = -1;
                return;
            }

            if (selectedPigQueueSecondaryIndex < 0)
            {
                if (workingPigQueue[selectedPigQueuePrimaryIndex].Color == workingPigQueue[queueIndex].Color)
                {
                    selectedPigQueueSecondaryIndex = queueIndex;
                    return;
                }

                selectedPigQueuePrimaryIndex = queueIndex;
                selectedPigQueueSecondaryIndex = -1;
                statusMessage = "Select a second pig with the same color to swap ammo.";
                return;
            }
        ...
        {
            EnsurePigQueueSelectionValid();
            primaryIndex = selectedPigQueuePrimaryIndex;
            secondaryIndex = selectedPigQueueSecondaryIndex;
            return IsValidPigQueueIndex(primaryIndex)
                && IsValidPigQueueIndex(secondaryIndex)
                && workingPigQueue[primaryIndex].Color == workingPigQueue[secondaryIndex].Color;
        }
        ...
        private void DrawSelectedPigQueueSwapPanel()
        {
            ...
            var amount = Mathf.Max(GetAmmoStep(), selectedPigQueueSwapAmount);
            amount = SnapDownToStep(amount, GetAmmoStep());
            selectedPigQueueSwapAmount = amount;
            ...
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                selectedPigQueueSwapAmount = EditorGUILayout.DelayedIntField("Transfer Amount", selectedPigQueueSwapAmount);
                selectedPigQueueSwapAmount = Mathf.Max(GetAmmoStep(), SnapDownToStep(Mathf.Max(GetAmmoStep(), selectedPigQueueSwapAmount), GetAmmoStep()));

                using (new EditorGUILayout.HorizontalScope())
                {
                    using (new EditorGUI.DisabledScope(!CanTransferAmmoBetweenPair(primaryIndex, secondaryIndex, selectedPigQueueSwapAmount)))
                    {
                        if (GUILayout.Button("A -> B", GUILayout.Width(72f)))
                        {
                            if (TransferAmmoBetweenPair(primaryIndex, secondaryIndex, selectedPigQueueSwapAmount))
                            {
                                statusMessage = $"{selectedPigQueueSwapAmount} ammo moved from A to B.";
                            }
                        }
                    }

                    using (new EditorGUI.DisabledScope(!CanTransferAmmoBetweenPair(secondaryIndex, primaryIndex, selectedPigQueueSwapAmount)))
                    {
                        if (GUILayout.Button("B -> A", GUILayout.Width(72f)))
                        {
                            if (TransferAmmoBetweenPair(secondaryIndex, primaryIndex, selectedPigQueueSwapAmount))
                            {
                                statusMessage = $"{selectedPigQueueSwapAmount} ammo moved from B to A.";
                            }
                        }
                    }
        ...
        {
            pigQueueDragSourceIndex = -1;
            pigQueueDragHoverLinearIndex = -1;
            pigQueueDragStartMousePosition = Vector2.zero;
            isPigQueueDragging = false;
        }
        ...
        private void HandlePigQueuePreviewInteraction(Rect previewRect, IReadOnlyList<PigQueuePreviewCell> previewCells)
        {
            ...
            if (current.type == EventType.MouseDown && current.button == 0)
            {
                ...
                {
                    pigQueueDragSourceIndex = pressedCell.QueueIndex;
                    pigQueueDragHoverLinearIndex = pressedCell.LinearIndex;
                    pigQueueDragStartMousePosition = current.mousePosition;
                    isPigQueueDragging = false;
                    GUI.FocusControl(null);
                    current.Use();
                }
            ...
            if (current.type == EventType.MouseDrag && pigQueueDragSourceIndex >= 0)
            {
                if (!isPigQueueDragging)
                {
                    isPigQueueDragging = (current.mousePosition - pigQueueDragStartMousePosition).sqrMagnitude
                        >= PigQueueDragStartDistance * PigQueueDragStartDistance;
                    if (isPigQueueDragging)
                    {
                        ClearPigQueueSelection();
                    }
                ...
                if (isPigQueueDragging)
                {
                    pigQueueDragHoverLinearIndex = TryGetPigQueuePreviewCellAtPosition(previewCells, current.mousePosition, false, out var hoveredCell)
            ...
            if (current.type == EventType.MouseUp && current.button == 0 && pigQueueDragSourceIndex >= 0)
            {
                if (isPigQueueDragging)
                {
                    if (TryGetPigQueuePreviewCellAtPosition(previewCells, current.mousePosition, false, out var hoveredCell))
                    {
                        if (hoveredCell.HasEntry
                            && SwapPigQueueEntries(pigQueueDragSourceIndex, hoveredCell.QueueIndex))
                        {
                            statusMessage = $"Swapped pigs #{pigQueueDragSourceIndex + 1} and #{hoveredCell.QueueIndex + 1}.";
                        }
                        else if (!hoveredCell.HasEntry
                            && MovePigQueueEntry(pigQueueDragSourceIndex, hoveredCell.LinearIndex))
                        {
                ...
                if (TryGetPigQueuePreviewCellAtPosition(previewCells, current.mousePosition, true, out var clickedCell)
                    && clickedCell.QueueIndex == pigQueueDragSourceIndex)
                {
        ...
        private void DrawPigQueueLanePreview(int holdingContainerCount, float layoutWidth)
        {
            ...
            for (int depthIndex = 0; depthIndex < maxLaneDepth; depthIndex++)
            {
                ...
                for (int laneIndex = 0; laneIndex < holdingContainerCount; laneIndex++)
                {
                    ...
                    if (isPigQueueDragging && pigQueueDragHoverLinearIndex == previewCell.LinearIndex)
                    {
                    ...
                    if (previewCell.HasEntry)
                    {
                        ...
                        if (isPigQueueDragging && queueIndex == pigQueueDragSourceIndex)
                        {
            ...
            if (isPigQueueDragging
                && IsValidPigQueueIndex(pigQueueDragSourceIndex)
                && Event.current.type == EventType.Repaint)
            {
                var dragCardRect = new Rect(
                    ...
                    cellWidth - 2f,
                    cardHeight - 2f);
                DrawPigQueuePreviewCard(dragCardRect, workingPigQueue[pigQueueDragSourceIndex], true, null);
            }
        }
        ...
        private bool TryResolveEnvironmentFromSceneContext(out PixelFlowEnvironmentContext environment, out string message)
        {
            environment = null;
            message = "Could not find PixelFlowSceneContext or PixelFlowEnvironmentContext in the open scene.";
            ...
            var environments = UnityEngine.Object.FindObjectsByType<PixelFlowEnvironmentContext>(FindObjectsInactive.Include, FindObjectsSortMode.None);
        ...
        private bool TryResolvePigBrush(PigColor color, out PixelFlowPlaceableDefinition definition)
        {
        ...
        private PixelFlowLevelData GetSelectedLevel()
        {
            if (selectedDatabase == null || selectedLevelIndex < 0 || selectedLevelIndex >= selectedDatabase.Levels.Count)
            {
                return null;
        ...
        private void TryAutoAssignDatabase()
        {
            ...
            var guids = AssetDatabase.FindAssets("t:PixelFlowLevelDatabase");
            ...
            selectedDatabase = AssetDatabase.LoadAssetAtPath<PixelFlowLevelDatabase>(assetPath);
        }
        ...
        private void TryAutoAssignBlockData()
        {
            ...
            var guids = AssetDatabase.FindAssets("t:PixelFlowBlockData");
            if (guids == null || guids.Length == 0)
            {
            ...
            var assetPath = AssetDatabase.GUIDToAssetPath(guids[0]);
            selectedBlockData = AssetDatabase.LoadAssetAtPath<PixelFlowBlockData>(assetPath);
        }

        private PixelFlowBlockData ResolveWorkingBlockData(PixelFlowBlockData preferred)
        {
            if (preferred != null)
            {
                return preferred;
        ...
        private PixelFlowBlockData CreateDefaultBlockDataAsset()
        {
            EnsureFolder(DefaultSettingsFolder);

            var blockData = AssetDatabase.LoadAssetAtPath<PixelFlowBlockData>(DefaultBlockDataAssetPath);
            if (blockData == null)
            {
                blockData = CreateInstance<PixelFlowBlockData>();
        ...
        private static bool HasMissingBlockPrefabReference(PixelFlowBlockData blockData)
        {
            if (blockData == null)
            {
                return false;
        ...
        private static bool TryAssignDefaultBlockPrefab(PixelFlowBlockData blockData)
        {
            if (blockData == null)
            {
                return false;
